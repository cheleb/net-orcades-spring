...
public class SpringGWTRemoteServlet extends RemoteServiceServlet {
	...
	public static final Logger LOGGER = Logger.getLogger(SpringGWTRemoteServlet.class);
	/**
	 * 
	 */
	...
	public String processCall(String payload) throws SerializationException {
        ...
        if (map.isEmpty()) {
            LOGGER.error("No implementation of [" + clazz.getName() + "] found in context");
            logRPCCall(Level.ERROR, request, method);
            throw new SerializationException("No implementation of [" + clazz.getName() + "] found in context");
        }
        //
        ...
        try {
            ...
            //The call.
            Object result = method.invoke(targetInjectedInstance, request.getParameters());

            if (LOGGER.isDebugEnabled()) {
                logResult(result);
            }
            
            return RPC.encodeResponseForSuccess(method, result, request.getSerializationPolicy());
        } catch (Throwable th) {
            logRPCCall(Level.ERROR, request, method);
            LOGGER.error(th);
            if (th instanceof InvocationTargetException) {
                InvocationTargetException invocationTargetException = (InvocationTargetException) th;
                LOGGER.error(clazz.getName() + "::" + method.getName() + "() failed!", invocationTargetException.getTargetException());
            }
            return RPC.encodeResponseForFailure(method, th, request.getSerializationPolicy());
    ...
    @SuppressWarnings("unchecked")
	private void logRPCCall(org.apache.log4j.Level priority, RPCRequest request, Method method) {
